...
export function registerVehicleTools(server: McpServer, service: VehicleService): void {
  server.registerTool(
    "search_vehicles",
    {
      ...
      inputSchema: { query: z.string().describe("Full or partial vehicle name, e.g. 'Constellation'") },
    },
    async ({ query }) => {
      const results = await service.search(query);
      if (results.length === 0) return { content: [{ type: "text", text: `No vehicles matched "${query}".` }] };
      return json(results);
    },
  );
  ...
  server.registerTool(
    "find_vehicle_purchase_locations",
    {
      ...
      inputSchema: { idVehicle: z.number().describe("Vehicle id from search_vehicles") },
    },
